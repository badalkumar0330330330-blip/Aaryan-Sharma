...
export function getZoyaSystemInstruction(creatorName: string = "Aaryan", role: string = "Boss", sassyLevel: number = 4, customNotes: string = "") {
...
export async function getZoyaResponse(prompt: string, history: { sender: "user" | "zoya", text: string }[] = []): Promise<string> {
  try {
    ...
    const systemInstruction = getZoyaSystemInstruction(prefs.creatorName, prefs.role, prefs.sassyLevel, prefs.customNotes);
    ...
    try {
      const prefs = await getUserPreferences();
      return `Uff, mera dimaag kharab ho gaya hai. Try again later, ${prefs.role}.`;
    } catch {
  ...
  try {
    ...
    const response = await ai.models.generateContent({
      ...
      config: {
        ...
        speechConfig: {
          voiceConfig: {
            prebuiltVoiceConfig: { voiceName: prefs.voiceName || "Kore" },
          },
        },
      },
    });
